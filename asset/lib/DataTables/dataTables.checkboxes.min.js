...
(function (factory) {
    ...
    Checkboxes.prototype = {
        ...
        onClickSelectAll: function (e, ctrl) {
            ...
            dt.column(colIdx, { page: ctx.aoColumns[colIdx].checkboxes && ctx.aoColumns[colIdx].checkboxes.selectAllPages ? "all" : "current", search: "applied" }).checkboxes.select(ctrl.checked);
            e.stopPropagation();
        },
        loadState: function () {
            var self = this;
        ...
        updateSelectAll: function (colIdx) {
            ...
            if (ctx.aoColumns[colIdx].checkboxes && ctx.aoColumns[colIdx].checkboxes.selectAll) {
                var cells = dt.cells("tr", colIdx, { page: ctx.aoColumns[colIdx].checkboxes.selectAllPages ? "all" : "current", search: "applied" });
                var $tableContainer = dt.table().container();
        ...
        showInfoSelected: function () {
            ...
            if (colIdx !== null) {
                ...
                var add = function ($el, name, num) {
                    $el.append($('<span class="select-item"/>').append(dt.i18n("select." + name + "s", { _: "%d " + name + "s selected", 0: "", 1: "1 " + name + " selected" }, num)));
                };
    ...
    Checkboxes.defaults = { stateSave: true, selectRow: false, selectAll: true, selectAllPages: true, selectCallback: null, selectAllCallback: null, selectAllRender: '<span style="display: none;">Select All</span><input type="checkbox" aria-label="Select All">' };

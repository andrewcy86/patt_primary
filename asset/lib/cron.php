...
function wppatt_switch_to_blog_cache_clear( $blog_id, $prev_blog_id = 0 ) {
	if ( $blog_id === $prev_blog_id )
		return;
  wp_cache_delete( 'notoptions', 'options' );
	wp_cache_delete( 'alloptions', 'options' );
}
add_action( 'switch_blog', 'wppatt_switch_to_blog_cache_clear', 10, 2 );
if(is_multisite()){
...
if(!$cron_job_schedule) {
	if (!(get_option('wppatt_db_version') < '2.0'))  {
		do_action('wppatt_cron');
	}
}

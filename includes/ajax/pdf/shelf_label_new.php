...
if (isset($_GET['dc'])) {
  ...
  if ($_GET['dc'] == 'W') {
    ...
    // JM - 9/9/2022 - Create a function to get all shelves from the db
    global $wpdb;
    $shelf_info = array();

    // - Refactor code from folder_seperator_sheet.php to accept multiple shelf_labels from the url
    //Function to get folderdocinfo_id from database
    //function fetch_folderdocinfo()
    function fetch_shelfid()
    {
      ...
      // JM - 9/11/2022 - Create SQL to fetch shelf_id's collected from the url.  
      $shelf_id_collection = $wpdb->get_results("
      SELECT shelf_id
      FROM " . $wpdb->prefix . "wpsc_epa_storage_status
      WHERE digitization_center = ". $dc ."
      AND shelf_id = ". $GLOBALS['id'] ."
      ");
  ...
  // JM - 09092022 - Create regex to get an array of shelfid's from the url
  if (preg_match('/^\d{1,3}A_\d{1,3}B_\d{1,3}S_\d{1,3}P_(E|W|ECUI|WCUI)$/i', $GLOBALS['id'])) {
  

    //Obtain array of Shelf ID's
    $shelf_info = fetch_shelfid();

    // JM - 09092022 - Add a comma boundary between every Shelf_id
    $shelf_info = explode(',', $GLOBALS['id']);

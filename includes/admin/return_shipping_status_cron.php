...
foreach ($shippingArray as $shippingCompany)  {
    ...
    switch ($shippingCompany) {
        case "external":
            ...
            WHERE tracking_number = UPPER('".WPPATT_EXT_SHIPPING_TERM_R3."') AND return_id <> -99999"
            );
            ...
            "SELECT *
            FROM " . $wpdb->prefix . "wpsc_epa_shipping_tracking
            WHERE tracking_number LIKE UPPER('%".WPPATT_EXT_SHIPPING_TERM."%') AND return_id <> -99999"
            );
            ...
            if (strtoupper($get_tracking_number) == strtoupper(WPPATT_EXT_SHIPPING_TERM_R3) && ($status_id == $status_decline_pending_cancel_term_id || $status_id == $status_decline_received_at_ndc_term_id) ) {
                $wpdb->update( $table_name, array( 'delivered' => 1),array('ID'=>$item->id));
            }
            //GET STATUS OF REQUEST
            if (strtoupper($get_tracking_number) == strtoupper(WPPATT_EXT_SHIPPING_TERM) && ($status_id == $status_decline_shipped_term_id || $status_id == $status_decline_shipped_back_term_id) ) {
                $wpdb->update( $table_name, array( 'shipped' => 1),array('ID'=>$item->id));
            }
            if (strtoupper($get_tracking_number) == strtoupper(WPPATT_EXT_SHIPPING_TERM) && ($status_id == $status_decline_pending_cancel_term_id || $status_id == $status_decline_received_at_ndc_term_id)  ) {
                $wpdb->update( $table_name, array( 'delivered' => 1),array('ID'=>$item->id));
            }
        ...
        case "usps":
            ...
            "SELECT *
            FROM " . $wpdb->prefix . "wpsc_epa_shipping_tracking
            WHERE company_name = 'usps' AND (shipped = 0 OR delivered = 0) AND return_id <> -99999"
            );
        ...
        case "fedex":
            ...
            "SELECT *
            FROM " . $wpdb->prefix . "wpsc_epa_shipping_tracking
            WHERE company_name = 'fedex' AND (shipped = 0 OR delivered = 0) AND return_id <> -99999"
            );
        ...
        case "ups":
            ...
            "SELECT *
            FROM " . $wpdb->prefix . "wpsc_epa_shipping_tracking
            WHERE company_name = 'ups' AND (shipped = 0 OR delivered = 0) AND return_id <> -99999"
            );
            ...
            foreach ($shipping_query as $item) {
                ...
                if($item->shipped == 0 && ($trackingNumber != '' || $trackingNumber != NULL)) {
        ...
        case "dhl":
            ...
            "SELECT *
            FROM " . $wpdb->prefix . "wpsc_epa_shipping_tracking
            WHERE company_name = 'dhl' AND (shipped = 0 OR delivered = 0) AND return_id <> -99999"
            );

...
FROM " . $wpdb->prefix . "wpsc_epa_storage_status
WHERE
digitization_center = ".$dc_west_tag->term_id."
");
...
$data_where = array('shelf_id' => $shelf_id, 'digitization_center' => $dc_west_tag->term_id);
$wpdb->update($wpdb->prefix.'wpsc_epa_storage_status', $data_update, $data_where);
...
if($position_count == 0) {
    ...
    $data_where_occupied = array('shelf_id' => $shelf_id, 'digitization_center' => $dc_west_tag->term_id);
...
//print_r($get_storage_box_ids_array);
//echo '<hr />';
//print_r($get_storage_location_array);
//echo '<hr />';
//print_r($result);

...
$get_ticketmeta = $wpdb->get_results("SELECT DISTINCT wpqa_wpsc_ticketmeta.ticket_id, request_id FROM wpqa_wpsc_ticketmeta 
									INNER JOIN wpqa_wpsc_ticket
									ON wpqa_wpsc_ticket.id = wpqa_wpsc_ticketmeta.ticket_id
									WHERE wpqa_wpsc_ticketmeta.ticket_id NOT IN 
						(SELECT ticket_id FROM wpqa_wpsc_ticketmeta 
                                    INNER JOIN wpqa_wpsc_ticket
									ON wpqa_wpsc_ticket.id = wpqa_wpsc_ticketmeta.ticket_id
                                    WHERE  ((wpqa_wpsc_ticket.active = 0) OR META_KEY = 'box_list_post_id' AND meta_value REGEXP'^[0-9]*$' AND meta_value <> '') )");
...
foreach($get_ticketmeta as $item) {
      ...
      $data_where = array('id' => $ticketmeta_ticket_id);
      $wpdb->update($wpdb->prefix . 'wpsc_ticket' , $data_update, $data_where);
      //do_action('wpsc_after_recycle', $ticketmeta_ticket_id);


      //BEGIN CLONING DATA TO ARCHIVE
      Patt_Custom_Func::send_to_archive($ticketmeta_ticket_id);

      //Archive audit log
      Patt_Custom_Func::audit_log_backup($ticketmeta_ticket_id);

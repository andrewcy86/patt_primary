...
if(
  ...
  //updates box status
  if( $old_bs != $bs ) {
    // Update current box status
    $data_update = array('box_status' => $bs);
    $data_where = array('id' => $box_id);
    $wpdb->update($table_name , $data_update, $data_where);
    
    if( $old_bs != $bs_waiting_shelved && $old_bs != $bs_waiting_rlo ) {
      // Update previous box status
      $data_update = array( 'box_previous_status' => $old_bs );
      $data_where = array( 'id' => $box_id );
      $wpdb->update( $table_name , $data_update, $data_where );
    }
  ...
  //echo '\n ' . $bs_waiting_shelved . ' - ' . $bs_waiting_rlo . '.';

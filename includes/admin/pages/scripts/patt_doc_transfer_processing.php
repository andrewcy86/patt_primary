...
$sel = mysqli_query($con,"SELECT COUNT(*) as allcount 
FROM " . $wpdb->prefix . "epa_patt_arms_logs
WHERE id <> -99999" . $stage_status . $overall_status . " AND 1 " . $searchQuery);
...
CONCAT(
	CASE 
	WHEN received_stage = 2 THEN CONCAT('<span>',folderdocinfofile_id,'</span>')
	ELSE CONCAT('<a href=\"admin.php?pid=docsearch&page=patttransferdetails&id=',folderdocinfofile_id,'\">',folderdocinfofile_id,'</a>') 
...
elseif($row['keyword_id_stage'] == 1) {
	// Success Status
	$keyword_success_icon = '<a class="truncate-text" data-toggle="tooltip" data-placement="top" data-html="true" data-original-title="Keyword Stage: Success"><span style="font-size: 1.3em; color: #2f631d;margin-left:4px;"><i class="fas fa-check-circle" aria-hidden="true" title="Success"></i><span class="sr-only">Success</span></span></a>
	<hr style="position: absolute;border: 0.1px solid gray;top: -2.5px;left: 75px;width: 2.3%;">';
}

...
switchFile("3807dc96-ec35-4ae6-8398-dcb7ddc1b021");switchInfo(\'' . $folderdocinfofile_id_initial . '\');
</script>';
}
...
if ($prev >= 0 && $prev < count($pages)) {
    ...
    echo '<button class="btn" onclick="getPaging(' . $pages[$prev] . ');switchInfo(\'' . $folderdocinfofile_id_prev . '\');switchFile(\'3807dc96-ec35-4ae6-8398-dcb7ddc1b021\');"><i class="fas fa-angle-left" aria-hidden="true" title="Previous"></i><span class="sr-only">Previous</span>  Previous</button>&nbsp;&nbsp;&nbsp;';
...
if ($next >= 0 && $next < count($pages)) {
    ...
    echo '<button class="btn" onclick="getPaging(' . $pages[$next] . ');switchInfo(\'' . $folderdocinfofile_id_next . '\');switchFile(\'3807dc96-ec35-4ae6-8398-dcb7ddc1b021\');">Next <i class="fas fa-angle-right" aria-hidden="true" title="Next"></i><span class="sr-only">Next</span></button>';
...
if (($box_status == $box_validation_tag->term_id || $box_status == $box_rescan_tag->term_id) && !in_array($box_ticket_status, $rescan_validate_status_id_arr) && ($folderdocinfofile_id_validation != 1) && ($folderdocinfofile_id_rescan != 1) && ($folderdocinfofile_id_object_key != NULL || $folderdocinfofile_id_object_key != '')) {
    echo '&nbsp;&nbsp;&nbsp;<button class="btn" id="next_validate" class="btn" onclick="executeValidation(\'' . $folderdocinfofile_id_current . '\');getPaging(' . $pages[$next] . ');switchInfo(\'' . $folderdocinfofile_id_next . '\');switchFile(\'3807dc96-ec35-4ae6-8398-dcb7ddc1b021\');">Next + Validate <i class="fas fa-angle-double-right" aria-hidden="true" title="Next and Validate"></i><span class="sr-only">Next and Validate</span></button>';
    //Rescan button
    echo '&nbsp;&nbsp;&nbsp;<button class="btn" id="next_rescan" class="btn" onclick="executeRescan(\'' . $folderdocinfofile_id_current . '\');getPaging(' . $pages[$next] . ');switchInfo(\'' . $folderdocinfofile_id_next . '\');switchFile(\'3807dc96-ec35-4ae6-8398-dcb7ddc1b021\');document.getElementById(\'paging\').contentWindow.location.reload();">Next + Re-Scan <i class="fas fa-angle-double-right" aria-hidden="true" title="Re-Scan"></i><span class="sr-only">Re-Scan</span></button>';
}
...
if (($box_status == $box_validation_tag->term_id || $box_status == $box_rescan_tag->term_id) && !in_array($box_ticket_status, $rescan_validate_status_id_arr) && ($folderdocinfofile_id_validation != 1) && ($folderdocinfofile_id_rescan != 1) && ($folderdocinfofile_id_object_key != NULL || $folderdocinfofile_id_object_key != '')) {
    echo '&nbsp;&nbsp;&nbsp;<button class="btn" id="next_validate" onclick="executeValidation(\'' . $folderdocinfofile_id_current . '\');getPaging(' . $pages[$next] . ');switchInfo(\'' . $folderdocinfofile_id_next . '\');switchFile(\'3807dc96-ec35-4ae6-8398-dcb7ddc1b021\');alert(\'' . $folderdocinfofile_id_current . ' has been validated\');location.reload();">Validate <i class="fas fa-check" aria-hidden="true" title="Validate Current"></i><span class="sr-only">Validate Current</span></button>';
    //Rescan button
    echo '&nbsp;&nbsp;&nbsp;<button class="btn" id="next_rescan" class="btn" onclick="executeRescan(\'' . $folderdocinfofile_id_current . '\');getPaging(' . $pages[$next] . ');switchInfo(\'' . $folderdocinfofile_id_next . '\');switchFile(\'3807dc96-ec35-4ae6-8398-dcb7ddc1b021\');alert(\'' . $folderdocinfofile_id_current . ' has been set to Re-Scan\');location.reload();">Re-Scan <i class="far fa-times-circle" aria-hidden="true" title="Re-Scan"></i><span class="sr-only">Re-Scan Current</span></button>';   
}
